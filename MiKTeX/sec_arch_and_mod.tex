...
Она заключается в отделении пользовательского интерфейса от логики приложения. Опишем каждый компонент данного архитектурного подхода. Схема представлена на рисунке~\ref{fig:arch:docs_connections}.
...
\begin{itemize}
  ...
  \item просмотр дополнительной информации (рейтинг, самая высокая стоимость, самая низкая стоимость);
...
\begin{figure}[H]
 \centering
   \includegraphics[scale=0.7]{data_diagram.png} 
   \caption{Структура слоя данных}
   \label{fig:arch:data_diagram}
\end{figure}
 
В качестве примера рассмотрим получение списка магазинов в приложении. Интерфейс вызывает UseCase GetStores, который представлен листингом~\ref{lst:getStores}.
...
Данный UseCase вызывает StoresGateway, представленный листингом~\ref{lst:storesGateway}, который осуществляет получение магазинов либо из базы данных, либо с сервера, попутно сохраняя их в базу данных. Данный метод проверяет наличие интернет подключение на устройстве, после этого делает запрос на сервер, либо в локальную базу данных, в зависимости от результата. Также выполняются преобразования моделей в модели подходящие приложению. Если интернет подключение на устройстве присутствует и получение списка магазинов с сервера произошло успешно, то метод сохраняет полученные магазины в локальную базу данных для дальнейшего использования.
...
StoresGateway применяет один из двух источников данных. При наличии подключения к интернету вызывается StoresDataSource, представленный листингом~\ref{lst:storesDataSource}, который делает запрос на сервер.
...
Интерфейс Api, представленный листингом~\ref{lst:storesApi}, реализуется с помощью библиотеки Retrofit, которая является одной из самых популярных и удобных библиотек для запросов на сервер на устройствах Android.
...
Если же интернет подключение отсутствует, то вызывается объект класса под названием StoresLocalDao, представленный листингом~\ref{lst:storesLocalDao}, который инкапсулирует логику для работы с локальной базой данных.
...
Данный поставщик содержимого не имеет конкретной реализации и используется для сохранения поисковых запросов пользователя. Данный поставщик включает в себя функционал удаления, добавления и обновления поисковых запросов. Поставщик работает исключительно с внутренним хранилищем Android, учитывая ограничения последних версий SDK.

\undersection{FileProvider}~\par
FileProvider является поставщиком содержимого который используется для доступа ко внутренним файлам приложения во время того как пользователь меняет свой аватар. Пример использования приведён листингом~\ref{lst:file_provider}.
...
Программная реализация включает OnboardingFragmentStateAdapter, представленная листингом~\ref{lst:onboarding_code}, который использует отдельный Fragment для каждой из страниц описания. При постраничном перелистывании данный адаптер вызывает отдельный Fragment с описанием конкретной функции используя позицию для обозначения нужного фрагмента.
...
При долистывании до конца или подтверждения пропуска приветствия приложение вызывает функцию launchMain, представленная листингом~\ref{lst:launch_main}, которая проверяет аутентифицирован ли пользователь. В зависимости от результата пользователь попадает на модуль авторизации, либо в главную часть приложения. Для запуска новых Activity, в приложении используются намерения.
...
AuthSignInFragment выполняет функцию аутентификации пользователя с помощью email и пароля. Пример пользовательского интерфейса представлен на рисунке~\ref{fig:arch:auth_2}.
...
Программная реализация содержит AuthSignInViewModel, который проводит базовую валидацию полей email и пароля с помощью ValidateEmail и ValidatePassword, представленных листингом~\ref{lst:add:a_6}.

При нажатии на кнопку Sign In клиентское приложение делает запрос на Firebase Auth с помощью logIn метода, представленным листингом~\ref{lst:auth_sign_in_uc}, который проверяет правильность введенных данных и делает запрос на сервер с этими данными.
...
Пользователь нажимая на кнопку Register вызывает метод register, представленный листингом~\ref{lst:auth_sign_up_uc}, который производит базовую валидацию полей и делает запрос на регистрацию пользователя в Firebase Auth.
...
Помимо списков, в реализации присутствуют отдельные методы для загрузки картинок с помощью библиотеки Glide. Реализовано множество методов для их загрузки с множеством опций в классе UriBindingAdapter, представленным листингом~\ref{lst:add:a_8}. Класс реализует одну из функций основных библиотек Android - DataBinding. Данная функциональность позволяет использовать методы прямо в коде интерфейса XML, что является удобным сокращением и повышает переиспользуемость методов.
...
С помощью нативного класса ShareCompat, был реализован метод по отправке ссылки на приложение. Ссылка может быть отправлена в любое приложение которое поддерживает тип test/plain, который представлен листингом~\ref{lst:share_method}.
...
Данная реализация позволяет иметь расширяемую с разных сторон реализацию. С помощью использования техники ViewType, данный список опций может иметь множество отображений под одним объектом списка, что значительно улучшает производительность на конечном устройстве. Количество ViewType соразмерно увеличивает количество типов объекта в списке, данный подход позволяет не создавать разные классы списков под отдельные объекты.
...
Каждая из настроек синхронизируется с Firestore для каждого пользователя, что позволяет сохранять состояние настроек на разных устройствах под одним аккаунтом. Например, для настройки Notification используется ключ notification под которым для каждого отдельного пользователя записывается состояние этой настройки с помощью методов putNotification и getNotification, представленных листингом~\ref{lst:prefs_notification}. Методы используют уникальный идентификатор каждого пользователя, чтобы эффективно хранить значения ключей под каждого пользователя в Firestore. С помощью уникального идентификатора методы проходятся по структуре документа в отдельный документ пользователя и, используя SetOptions, проводится структурное объединение настроек пользователя на устройстве с настройками пользователя на сервере.
При использовании данных методов предусмотрено обрабатывание ошибок, так со стороны подключения к сети, так и со стороны библиотеки и правил безопасности описанных на стороне сервера. Для того чтобы методы были неблокирующими для UI пользователя, использована конструкция suspendCoroutine.
...
На экране информации пользователь может прочитать о приложении и перейти по ссылкам чтобы получить связанную с разработчиком информацию, например на сервер Discord или в PlayStore. Переход по ссылкам осуществляется с помощью browserIntent, намерения с помощью которого система открывает ссылку в доступном браузере с использованием ACTION VIEW типа намерения, представленному листингом~\ref{lst:prefs_intent}.
...
Для обеспечения безопасности чувствительных данных была использована Android библиотека Security для шифрования пользовательских настроек приложения. Логика по взаимодействию с этой библиотекой реализована в классе PreferenceDataSource, представленному листингом~\ref{lst:add:a_9}.

На серверной стороне для Firebase Storage и Firestore были написаны правила, которые не позволяют пользователям получать доступ ко всем данным, а только для их части, что соответствует их уникальному идентификатору, представленному листингом~\ref{lst:firebase_security}.
...
\begin{itemize}
   ...
   \item приложение выключает магазин в базе данных;
...
\begin{itemize}
   ...
   \item приложение включает магазин в базе данных;
   \item приложение запоминает выбор пользователя.
\end{itemize}
